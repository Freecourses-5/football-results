...
const FIXTURES_CACHE_MS = Number(process.env.FIXTURES_CACHE_MS || 1800000); // 30 min
const LIVE_CACHE_MS = Number(process.env.LIVE_CACHE_MS || 600000); // 10 min
...
async function footballApi(endpoint, params = {}, cacheKey, ttl) {
  ...
  if (!API_KEY) {
    const err = new Error("API_FOOTBALL_KEY is missing. Create .env from .env.example.");
    err.status = 500;
    throw err;
  }
...
app.listen(PORT, () => {
  console.log(`Football Results running on http://localhost:${PORT}`);
});
